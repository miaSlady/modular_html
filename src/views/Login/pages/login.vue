  ...
  <div class="box">
    ...
    <div class="rightBox">
      ...
      <div class="form">
        <div class="item">
          <p>账号：</p>
          <input v-model.trim="loginParams.account" class="input1" type="text">
        </div>
        <div class="item">
          <p>密码：</p>
          <input v-model.trim="loginParams.userPwd" maxlength="12" class="input1" type="password">
        </div>
        <div class="item">
          <p>验证码：</p>
          <input v-model.trim="loginParams.code" class="input1 input2" maxlength="4" @keyup.enter="handleLogin">
          <span @click="getCode" class="code" v-html="codeImg"></span>
        </div>
        <div class="btn" :class="{loginNow:loading}" @click="handleLogin">登录</div>
...
import {Login,GetCode} from '../assets/js/getData'
export default {
  data (){
    return {
      codeImg:'',
  ...
  methods:{
    ...
    handleLogin(){
      ...
      if(!this.loginParams.userPwd){
        this.$message.error("请输入密码");
        return;
      }
      if(this.loginParams.userPwd.length<6){
      ...
      this.loading=true;
      Login(this.loginParams).then(msg=>{
        this.loading=false;
        if(msg.success){
          let data=msg.data;
